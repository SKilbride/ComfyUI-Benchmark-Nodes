...
class WorkflowManager:
    ...
    def get_safetensors_files(self):
        """
        Scan all workflow node inputs for .safetensors filename references.

        Returns a list of dicts: {filename, node_id, class_type, title, input_key}
        """
        if self.workflow is None:
            return []
        results = []
    ...
    def validate_manifest_safetensors(self, manifest_path):
        """
        Validate that every .safetensors file referenced in the workflow is listed
        in the manifest.  The manifest may contain more files than the workflow uses
        (that is fine); the reverse is an error.

        Args:
            manifest_path (Path): Path to the extracted manifest.yaml / manifest.json.

        Returns:
            (is_valid: bool, missing: list[dict])
        """
        ...
        try:
            ...
            self.log(f"Warning: Could not load manifest for safetensors validation: {e}")
            return True, []

        def _effective_safetensors_name(item):
            """Return the effective local .safetensors filename for a manifest item."""
            # New compact HF format: filename lives at the end of remote_path
            if item.get("source") == "huggingface" and "repo" not in item:
                rp = item.get("remote_path", "")
                if "/tree/" in rp:
                    hf_path = rp.split("/tree/", 1)[1].split("/", 1)[-1]
                    name = Path(hf_path).name
                    if name.lower().endswith(".safetensors"):
                        return name
            # path field is a full file path (old format, or explicit new format)
            path_str = item.get("path", "")
            if path_str and Path(path_str).suffix:
                name = Path(path_str).name
                if name.lower().endswith(".safetensors"):
                    return name
            # Old HF format: explicit 'file' field
            file_field = item.get("file", "")
            if file_field and file_field.lower().endswith(".safetensors"):
                return file_field
            return None

        # Collect every .safetensors filename declared in manifest items
        manifest_filenames = set()
        for item in manifest.get("items", []):
            name = _effective_safetensors_name(item)
            if name:
                manifest_filenames.add(name)

        workflow_files = self.get_safetensors_files()
        missing = [f for f in workflow_files if f["filename"] not in manifest_filenames]
        is_valid = not missing

        self.log("\n--- Safetensors Manifest Validation ---")
        self.log(f"Workflow references {len(workflow_files)} safetensors file(s); "
                 f"manifest declares {len(manifest_filenames)}.")
        if is_valid:
            self.log("✓ All workflow safetensors are present in the manifest.")
        else:
            self.log(f"⚠ WARNING: {len(missing)} workflow safetensors file(s) NOT found in manifest:")
            for f in missing:
                self.log(f"  ✗ {f['filename']}  "
                         f"(node {f['node_id']}: {f['class_type']} — {f['title']})")
        self.log("---------------------------------------\n")
