...
def integrate_manifest_from_folder(
    folder_path: Path,
    comfy_path: Path,
    log_file: Optional[Path] = None,
    skip_existing: bool = True,
    ...
    """
    Process a manifest.yaml directly from an already-extracted folder (no ZIP).

    Mirrors integrate_manifest_with_package_manager but reads the manifest
    file directly from disk rather than extracting it from a ZIP archive.
    ...
    # Probe for manifest at folder root
    manifest_file = None
    for name in ("manifest.yaml", "manifest.yml", "manifest.json"):
        candidate = folder_path / name
        if candidate.exists():
            manifest_file = candidate
            print(f"[manifest_integration] Detected manifest in folder: {name}")
            break
    ...
    _log("\n" + "=" * 60)
    _log("PROCESSING MANIFEST FROM FOLDER")
    _log("=" * 60)
    ...
    try:
        ...
        # Retry loop for gated-model auth — headless version (no GUI dialog)
        max_retries = 3
        attempt = 0
        download_success = False
        ...
        while attempt < max_retries:
            try:
                _log(f"[manifest_integration] Starting downloads (Attempt {attempt + 1}/{max_retries})...")
                handler.download_items(
                    skip_existing=skip_existing,
                    required_only=False,
                    verify_checksums=verify_checksums,
                ...
                err_str = str(e)
                if "HF_TOKEN" in err_str or "gated model" in err_str or "401" in err_str or "403" in err_str:
                    _log(f"[manifest_integration] Authentication required: {e}")
                    # Headless: check env var instead of showing GUI dialog
                    token = os.environ.get("HF_TOKEN", "").strip()
                    if token:
                        _log("[manifest_integration] Using HF_TOKEN from environment. Retrying...")
                        handler.hf_token = token
                        attempt += 1
                        continue
                    else:
                        _log("[manifest_integration] No HF_TOKEN in environment. Set HF_TOKEN to access gated models.")
                        raise
                else:
                    raise
        ...
        custom_nodes_installed = handler.custom_nodes_were_downloaded()
        if custom_nodes_installed:
            _log("[manifest_integration] Custom nodes installed via manifest")
        else:
            _log("[manifest_integration] Custom nodes already up-to-date")

        # Merge NUM_INSTANCES / GENERATIONS from manifest config if present
        config = manifest_config or {}
        manifest_cfg = config.get("config", config)  # handle nested or flat
        result_config = dict(manifest_config)
        if "NUM_INSTANCES" in manifest_cfg:
            result_config["NUM_INSTANCES"] = manifest_cfg["NUM_INSTANCES"]
        if "GENERATIONS" in manifest_cfg:
            result_config["GENERATIONS"] = manifest_cfg["GENERATIONS"]

        _log("[manifest_integration] Manifest processing complete!")
        _log("=" * 60 + "\n")
        return True, result_config, custom_nodes_installed

    except Exception as e:
        print(f"[manifest_integration] Error processing folder manifest: {e}")
        if "HF_TOKEN" not in str(e):
            import traceback
            traceback.print_exc()
        return False, None, False
